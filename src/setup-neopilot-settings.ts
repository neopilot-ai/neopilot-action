...
export async function setupNeopilotSettings(
  settingsInput?: string,
  homeDir?: string,
) {
  const home = homeDir ?? homedir();
  const settingsPath = `${home}/.neopilot/settings.json`;
  console.log(`Setting up Neopilot settings at: ${settingsPath}`);

  // Ensure .neopilot directory exists
  console.log(`Creating .neopilot directory...`);
  await $`mkdir -p ${home}/.neopilot`.quiet();
